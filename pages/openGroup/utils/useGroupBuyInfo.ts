...
export function useGroupBuyInfo(
  groupId: string,
  isReady: boolean,
  ...
  const [groupBuyObject, setGroupBuyObject] = useState<
    undefined | null | GroupBuyObject
  >(undefined);
  ...
  const myOrder: UserOrder | undefined = useMemo(() => {
    ...
    if (!userInfo || !groupBuyObject || !groupBuyObject.userOrderList?.length)
      return undefined;
    else {
      return groupBuyObject.userOrderList.find(
        (userOrder) => userOrder.user.loginId === userInfo.loginId,
      );
    }
  ...
  async function loadGroupBuy(groupId: string) {
    ...
    try {
      ...
      setErrorMessage(
        `載入團單發生錯誤！ 錯誤代碼：${ErrorCode['載入團單資訊發生錯誤']} , 請重新嘗試或來信回報`,
      );
      serverUtils.addLog(
        `${error}`,
        LoggingLevel['FATAL'],
        ErrorCode['載入團單資訊發生錯誤'],
      );
      setGroupBuyObject(null);
    }
    setLoadingLock(false);
  ...
  const updateGroupState = useCallback(
    (type: GroupBuyStatus) => {
      ...
      async function groupState(
        ...
        setLoadingLock(true);
        try {
          const changeTime: string = await serverUtils.updateGroupState(
            groupBuyObject.uid,
            type,
          );
          if (mounted.current) {
          ...
          setErrorMessage(
            `狀態變更失敗! 錯誤代碼: ${ErrorCode['團單變更狀態失敗']} ，請再試一次或是來信回報`,
          );
          const errorLog = `團單變更狀態失敗 >> groupId = ${
            groupBuyObject.uid
          } ，從${groupBuyObject.statusText} 狀態改成 :${getKeyByValue(
            GroupBuyStatus,
            type,
          )}，錯誤訊息: ${error}`;
          serverUtils.addLog(
  ...
  const receiptOrder = useCallback(
    async (order: UserOrder) => {
      if (userInfo) {
        setLoadingLock(true);
        const newState =
          order.user.loginId === userInfo.loginId
            ? ReceiptType['已簽收完畢']
  ...
  const deleteMyOrder = useCallback(async () => {
    if (myOrder) {
      ...
      try {
        ...
        setErrorMessage(
          `刪除訂單失敗! 錯誤代碼: ${ErrorCode['刪除訂單失敗']} ，請再試一次或是來信回報`,
        );
  ...
  const saveOrder = useCallback(
    async (newOrder: UserOrder) => {
      ...
      try {
        ...
        try {
          if (groupBuyObject?.store && groupBuyObject.store.editable) {
            ...
            const newMenuData = newOrder.orderList.filter((goods) => {
              ...
              if (data) {
                ...
                return goods.appendTermList.some((append) => {
                  data.appendMenu.find(
                    (t) => t.name === append.name && t.money === append.money,
                  ) === undefined;
                });
              } else {
                return true; // 新增的
            ...
            if (newMenuData.length > 0) {
              // 有被更動的 => 去改store的資料
              await serverUtils.updateStoreMenuData(
                groupBuyObject.store,
                newMenuData,
              );
            }
          }
        } catch (error) {
          const errorLog = `更改商家MenuData失敗 >> storeUid = ${groupBuyObject
            ?.store?.uid}, newOrder =${newOrder.stringify()}}
                ，錯誤訊息: ${error}`;
          serverUtils.addLog(
            errorLog,
            LoggingLevel['ERROR'],
        ...
        setErrorMessage(
          `儲存訂單失敗! 錯誤代碼: ${ErrorCode['儲存訂單失敗']} ，請再試一次或是來信回報`,
        );
