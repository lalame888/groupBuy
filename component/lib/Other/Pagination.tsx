...
export function MyPagination(props: PaginationProps) {
  ...
  return (
    <div
      style={{ margin: '30px 0px', display: 'flex', justifyContent: 'center' }}
    >
      <Pagination>
        {props.nowPageNumber > 1 && (
          <Pagination.First
            onClick={() => {
              toPage(Math.max(props.nowPageNumber - 10, 1));
            }}
          />
        )}
        {props.nowPageNumber > 1 && (
          <Pagination.Prev
            onClick={() => {
              toPage(props.nowPageNumber - 1);
            }}
          />
        )}
        {Array.from(
          Array(Math.min(12, props.maxPageNumber)).keys(),
          (i) => i,
        ).map((number) => {
          const showPage =
            basicNumber === 0 ? basicNumber + number + 1 : basicNumber + number;
          if (showPage > props.maxPageNumber) {
            return null;
          }
          if (basicNumber === 0 && showPage === 12) {
            return null;
          }
        ...
        {props.nowPageNumber < props.maxPageNumber && (
          <Pagination.Last
            onClick={() => {
              toPage(Math.min(props.nowPageNumber + 10, props.maxPageNumber));
            }}
          />
        )}
      </Pagination>
