...
export function MenuCanvas(props: MenuCanvasProps) {
  ...
  function quickKey(this: Window, event: KeyboardEvent): void {
    ...
    if (
      target.classList.contains('modal') === false && // 不是正在操作modal的時候
    ...
    <div style={{ maxWidth: '90%', maxHeight: '80vh', ...props.style }}>
      {hasPagination && (
        ...
        >
          <span style={{ fontSize: '18px', marginBottom: '10px' }}>
            第 {index + 1} 頁 / 共 {props.menuImage.length} 頁
          </span>
        </div>
      )}
      <div style={{ display: 'flex', position: 'relative' }}>
        ...
        <Cropper
          ...
          style={{
            left: hasPagination ? 'calc(max(10%,80px))' : '0px',
            position: 'relative',
            width: '100%',
          }}
          dragMode="move" // 移動模式
