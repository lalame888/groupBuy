        ...
        <>
          {props.isEditAble && (
            <Confirm
              ...
              onConfirm={() => {
                changeStatus(GroupBuyStatus['結單中']);
              }}
              title={'結算團單'}
              text={`確定要${props.endTime ? '提前' : ''}結算此份團單嗎？\n
                        團單將會轉為「結單中」狀態，其他人將無法新增訂單與編輯訂單。`}
            >
              <MyHoverButton style={buttonStyle} disabled={props.loadingLock}>
                結算團單
              </MyHoverButton>
            </Confirm>
          )}
          {!props.isEditAble && (
            <Confirm
              ...
              text={'確定完成此份團單嗎？\n團單將會轉為完成狀態。'}
              onConfirm={() => {
                changeStatus(GroupBuyStatus['已完成']);
              }}
            >
              <MyHoverButton style={buttonStyle} disabled={props.loadingLock}>
                完成團單
          ...
          <Dropdown style={toggleStyle}>
            <Dropdown.Toggle
              as="span"
              className={moduleStyles['custom-toggle']}
            >
              <IconButton
                icon={faEllipsisV}
                style={{ fontSize: '20px' }}
            ...
            <Dropdown.Menu style={{ fontSize: '16px' }}>
              ...
              {!props.isEditAble && (
                <Dropdown.Item
                  onClick={() => {
                    changeStatus(GroupBuyStatus['開放跟團中']);
                  }}
                >
                  恢復開放跟團
                </Dropdown.Item>
          ...
          <div style={{ display: 'none' }}>
            ...
            <Confirm
              disabled={props.loadingLock}
              onConfirm={() => {
                changeStatus(GroupBuyStatus['已取消團單']);
              }}
              title={'刪除團單'}
              text={'確定要作廢此份團單嗎？\n團單將會轉為取消狀態。'}
              confirmType="delete"
