        ...
        props.orderList.forEach((userOrder: UserOrder)=>{
            userOrder.orderList.forEach((goods)=>{
                ...
                if (map[goods.name].list[goods.appendTermText] === undefined) {
                    map[goods.name].list[goods.appendTermText] = {appendText: goods.appendTermText, noteList:[],number: 0, money: 0};
                }
                ...
                {goodsList.map((goods,index)=>{
                    ...
                    return(
                        ...
                        {list.map((term,i)=>
                            ...
                            <td style={hoverStyle(index)} >{term.noteList.map((note,key)=>
                                <p key={key} style={{marginBottom:'5px',...hoverStyle(index)}}>{note}</p>
                            )}</td>
                        </tr>
                        )}
                        </React.Fragment>
