...
function appendTermLabel(value: AppendTerm, zeroShow?: boolean): string {
  if (zeroShow && value.money === 0) {
    ...
    return `${value.name}${` (${value.money > 0 ? '+' : ''}${
      value.money
    } 元)`}`;
  } else {
    return value.name;
...
export function EditGoodsCard(props: EditGoodsCardProps) {
  const menuList: Array<SelectOption<GoodsData>> = useMemo(() => {
    return props.menu.map((m) => ({
      value: new GoodsData(m.name, m.money),
      label: m.name,
    }));
  }, [props.menu]);
  ...
  const [orderNumber, setOrderNumber] = useState<string>(
    props.goods.number.toString(),
  );
  const [money, setMoney] = useState<string>(props.goods.money.toString());
  const [appendTerms, setAppendTerms] = useState<
    Array<SelectOption<AppendTerm>>
      ...
      <div style={{ padding: '10px 40px 10px 10px', wordBreak: 'keep-all' }}>
        ...
        <p style={cardRowStyle}>
          ...
          <CreatableSelect
            ...
            isValidNewOption={(inputValue) =>
              props.setting.canNewTerm && inputValue !== ''
            }
        ...
        <p style={cardRowStyle}>
          ...
          <OverlayTrigger
            ...
            onToggle={(nextShow) =>
              nextShow && props.setting.canNewTerm && props.goods?.name
            }
          >
            <CreatableSelect
              ...
              isClearable={false}
              placeholder={
                props.goods?.name ? '可選擇特製項目' : '請先選擇項目名稱'
              }
              options={appendList}
              value={appendTerms}
              onChange={(newValue) => {
              ...
              onCreateOption={(newValue) => {
                if (newValue !== null) {
                  const newAppendTerm: AppendTerm =
                    typeof newValue === 'string'
                      ? splitNameAndMoney(newValue)
                      : newValue;
                  setAppendTerms((list) => [
                    ...list,
                    turnToSelectOption(newAppendTerm),
                  ]);
                }
              }}
              noOptionsMessage={() =>
                props.setting.canNewTerm ? '輸入文字以新增項目' : '無此項目'
              }
