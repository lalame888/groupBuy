...
export class UserOrder extends DataSetter<UserOrder,UserOrderData>{
  ...
  static loadUserOrder(data: UserOrderData, orderList: Array<Partial<GoodsData>>): UserOrder {
      const orders = orderList.map((order)=> {
          const { name, money, number, note, appendTermList,uid, isNoGoods } = order; // 把物件變成class
          return new GoodsData(name||'', money||0, number, note, appendTermList, uid, isNoGoods);
      })
  ...
  get appendMoney(): number {
    const edited = this.editArray.find((edit)=> edit.type === EditOrderAction['修改額外費用']);
    if (edited) return edited.value as number;
    return this.dataGetter('appendMoney') as number
  }
  get payMoney(): number {
    const edited = this.editArray.find((edit)=> edit.type === EditOrderAction['已付金額']);
    if (edited) return edited.value as number;
    return this.dataGetter('payMoney') as number
  }
  get receipt(): ReceiptType {
    const edited = this.editArray.find((edit)=> edit.type === EditOrderAction['收貨狀態']);
  ...
  get orderContainText(): string { // 只顯示沒有標示成缺貨的
    if (this.orderList.length ===0) return '尚無內容';
    return this.orderList.filter((o)=> !o.isNoGoods).map((order)=> order.orderContainText).join('、')
  }

  get noGoodsContainText(): string { // 缺貨的內容
