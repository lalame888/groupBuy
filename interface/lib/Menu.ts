...
export class GoodsData extends Cloneable<GoodsData> {
  ...
  get orderContainText(): string {
    return `${this.name}${
      this.appendTermList.length
        ? `(${this.appendTermList.map((append) => append.name).join('＆')})`
        : ''
    } * ${this.number}`;
  }
  ...
  get appendTermText(): string {
    ...
    return this.appendTermList
      .map(
        (append) =>
          `${append.name}${append.money > 0 ? ` (+${append.money}$)` : ''}`,
      )
      .join('、');
  }
}
