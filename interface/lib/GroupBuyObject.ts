...
export class GroupBuyObject extends DataSetter<GroupBuyObject, GroupBuyData> {
  ...
  static loadObject(prams: LoadGroupData): GroupBuyObject {
    const result = new GroupBuyObject(prams.builder, prams.data);
  ...
  // 定義getter
  get title(): string {
    return this.dataGetter('title') as string;
  }
  get uid(): string {
  ...
  get store(): StoreObject | undefined {
    return this.dataGetter('store') as StoreObject;
  }
  get userOrderList(): Array<UserOrder> | undefined {
    return this.userOrder;
  }
  get setting(): GroupSetting {
    return this.dataGetter('setting') as GroupSetting;
  }
  ...
  get isEnd(): boolean {
    return (
      this.data.statues === GroupBuyStatus['已完成'] ||
      this.data.statues === GroupBuyStatus['已取消團單']
    );
  }
  ...
  get endTime(): Date | undefined {
    const time = this.data.deleteTime
      ? this.data.deleteTime
      : this.data.setting.endTime;
